...
import { notify, formatCurrency, formatTime, escapeHtml, confirmModal } from './utils.js';
...
export const Telemetry = {
    init: () => {
        ...
        globalState.chartInstance = new Chart(ctx, {
            type: 'line',
            data: { labels: Array(20).fill(''), datasets: [{ label: 'Temp (°C)', data: Array(20).fill(null), borderColor: '#f59e0b', backgroundColor: 'rgba(245, 158, 11, 0.1)', borderWidth: 2, fill: true, tension: 0.4, pointRadius: 2 }] },
            options: { responsive: true, maintainAspectRatio: false, animation: { duration: 0 }, scales: { y: { beginAtZero: false, min: 0, max: 100 }, x: { display: false } }, plugins: { legend: { display: false } } }
        });
    },
    updateFromPayload: (machineId, controls) => {
        ...
        if (globalState.chartInstance) {
            ...
            data.shift(); data.push(newVal); globalState.chartInstance.update();
        }
        const kpi = document.getElementById('kpi-temp');
        if (kpi) { kpi.textContent = newVal.toFixed(1) + '°C'; kpi.style.color = newVal > 85 ? '#ef4444' : '#f59e0b'; }
        ...
        if (bar) { bar.style.width = Math.min(newVal, 100) + '%'; bar.style.background = newVal > 85 ? '#ef4444' : (newVal > 60 ? '#f59e0b' : '#3b82f6'); }
    }
};
...
export const Vision = {
    init: () => {
        ...
        if (storedUrl) { const input = document.getElementById('ngrok-url-input'); if (input) input.value = storedUrl; }
    },
    connect: () => {
        ...
        if (status) { status.className = 'status-pill on'; status.innerHTML = '<span class="status-pill dot"></span>Conectado'; }
...
export const MachineControl = {
    ...
    toggleSwitch: async (id, key) => { 
        ...
        try {
            ...
            } else if (id === 2) c.calentador_on = (key === 'heat_on');
    ...
    toggleEmergency: () => {
        if (!globalState.isEmergencyActive) { 
            confirmModal('PARO DE EMERGENCIA', '¿Detener TODO?', async () => { 
                globalState.isEmergencyActive = true; document.body.classList.add('emergency-mode'); 
                const btn = document.getElementById('btn-global-stop'); if(btn) { btn.classList.add('active'); btn.innerHTML = 'RESTABLECER'; } 
                await MachineControl.sendCommand(1, 'Paro'); await MachineControl.toggleSwitch(2, 'heat_off'); 
                Dashboard.logEvent(0, 'PARO GLOBAL', 'ERROR');
            }, 'btn-primary-modal-danger', 'PARAR'); 
        } else { 
            confirmModal('Restablecer', '¿Reactivar?', async () => { 
                globalState.isEmergencyActive = false; document.body.classList.remove('emergency-mode'); 
...
export const Dashboard = {
    renderMachines: async (userRole) => {
        ...
        data.forEach(m => {
            ...
            if (m.id === 1) {
                ...
                body = `<div class="m-area"><i class="fa-solid fa-microchip"></i> PLC M1</div>${controlsHTML}`;
            } else if (m.id === 2) {
                ...
                const controlsHTML = isAdmin ? `
                    <div class="machine-interface" style="margin-top: 20px;">
                        <div class="control-group"><span class="control-label">Calefacción</span>
                            <div class="segmented-control">
                                <div class="segmented-option"><input type="radio" name="heat" id="m2_heat_off" ${!isHeating ? 'checked' : ''} onclick="window.MachineControl.toggleSwitch(2,'heat_off')"><label for="m2_heat_off">OFF</label></div>
                                <div class="segmented-option"><input type="radio" name="heat" id="m2_heat_on" ${isHeating ? 'checked' : ''} onclick="window.MachineControl.toggleSwitch(2,'heat_on')"><label for="m2_heat_on">ON</label></div>
                            </div>
                        </div>
                    </div>` : '';
                body = `<div class="clean-gauge"><div class="gauge-readout" id="gauge-m2-val">${temp.toFixed(1)}<span class="gauge-unit">°C</span></div><div class="gauge-bar-bg"><div id="temp-bar-2" class="gauge-bar-fill" style="width:${Math.min(temp, 100)}%; background:${temp > 85 ? '#ef4444' : '#3b82f6'}"></div></div></div>${controlsHTML}`;
            }
    ...
    createEmployee: async (e) => {
        ...
        const { data, error } = await db.auth.signUp({ email: document.getElementById('new-user-email').value, password: document.getElementById('new-user-pass').value });
        if (!error && data.user) {
            await db.from('perfiles').upsert([{ id: data.user.id, email: data.user.email, nombre_completo: document.getElementById('new-user-name').value, rol: document.getElementById('new-user-role').value, area: document.getElementById('new-user-dept').value }]);
            notify.close(load); notify.success('Creado'); document.getElementById('modal-create-user').style.display='none'; Dashboard.initAdminUsers(globalState.userProfile.rol);
        } else { notify.close(load); notify.error(error?.message); }
    },
    ...
    // --- BITÁCORA (CORREGIDO CON ERROR HANDLING) ---
    logEvent: async (mid, evt, type, val=null) => {
        await db.from('bitacora_industrial').insert({ maquina_id: mid, evento: evt, tipo: type, usuario: globalState.userProfile?.nombre_completo || 'Sys', valor_lectura: val });
    },
    renderReports: async () => {
        ...
        try {
            ...
            if (error) throw error;
            if (!logs || logs.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="text-center">Sin actividad reciente.</td></tr>'; return; }
    ...
    // --- VENTAS (CORREGIDO CON ERROR HANDLING Y JOIN SEGURO) ---
    renderSales: async (filter = 'todos') => {
        ...
        try {
            ...
            if (error) throw error;
            if (!orders || orders.length === 0) { tbody.innerHTML = '<tr><td colspan="5" class="text-center">No hay pedidos.</td></tr>'; return; }
            ...
            tbody.innerHTML = orders.map(o => {
                ...
                // Protección contra perfiles nulos
            ...
            console.error(e);
            tbody.innerHTML = `<tr><td colspan="5" class="text-center" style="color:red">Error cargando ventas: ${e.message}. Verifique permisos RLS o relaciones en Supabase.</td></tr>`;
        }
    },
    updateOrderStatus: (orderId, newStatus) => {
        confirmModal('Actualizar Pedido', `¿Cambiar estado a ${newStatus}?`, async () => {
            ...
            if(newStatus === 'Enviado') updates.tracking_info = { carrier: 'EcoLogistics', tracking_number: 'TRK-' + Date.now().toString().slice(-6), history: [{status: 'Recolectado', date: new Date().toISOString(), completed: true}] };
    ...
    loadChatMessages: async (channel) => {
        ...
        list.innerHTML = '';
        if(data?.length) { [...data].reverse().forEach(m => Dashboard.renderChatMessage(m, false)); list.scrollTop = list.scrollHeight; }
        else list.innerHTML = '<div style="text-align:center;padding:20px;color:#ccc">Canal vacío</div>';
    },
    sendMessage: async () => {
        const inp = document.getElementById('chat-input-text');
        if(!inp.value.trim()) return;
        await db.from('mensajes').insert({ mensaje: inp.value, sender: globalState.userProfile.nombre_completo, role: globalState.userProfile.rol, canal: globalState.currentChannel });
        inp.value = '';
    },
    renderChatMessage: (m, anim=true) => {
        ...
        list.insertAdjacentHTML('beforeend', `<div class="msg-item" style="${anim?'animation:fadeIn 0.3s':''} ${isMe?'background:#eff6ff;border-color:#bfdbfe':''}"><div class="msg-avatar" style="${isMe?'background:#3b82f6;color:white':''}">${m.sender[0]}</div><div style="flex:1"><strong>${escapeHtml(m.sender)}</strong><p style="margin:0">${escapeHtml(m.mensaje)}</p></div></div>`);
