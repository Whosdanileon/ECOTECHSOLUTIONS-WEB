...
console.log('🚀 EcoTech System v3.1: High Performance UI');
...
const AppUI = {
    refreshState: async (user) => {
        ...
        if (path.includes('panel.html')) {
            await AppUI.handlePanelPage(user);
        } else if (path.includes('cuenta.html')) {
            await AppUI.handleAccountPage(user);
        } else if (path.includes('checkout.html')) {
            await AppUI.handleCheckoutPage(user);
        }
    },

    // --- FIX: Panel Anti-Flicker ---
    handlePanelPage: async (user) => {
        const overlay = document.getElementById('login-overlay');
        const loginBox = document.querySelector('.login-box'); // Referencia a la caja interna
        const layout = document.getElementById('dashboard-layout');

        const isStaff = user && CONFIG.ROLES.STAFF.includes(globalState.userProfile?.rol);

        if (!isStaff) {
            // NO LOGUEADO: Mostrar overlay y caja
            if (layout) layout.style.display = 'none';
            if (overlay) {
                overlay.style.display = 'flex';
                // Animación suave para mostrar la caja de login solo cuando confirmamos que no hay usuario
                if (loginBox) {
            ...
            // LOGUEADO: Ocultar overlay inmediatamente
    ...
    handleCheckoutPage: async (user) => {
        ...
        if (!user) {
            ...
            if(itemsDiv) {
                itemsDiv.innerHTML = `<div style="display:flex;justify-content:space-between; padding:10px 0; border-bottom:1px solid #eee;">
                    <span>${prod.nombre} x${qty}</span><strong>${formatCurrency(total)}</strong>
                </div>`;
            }
    ...
    performRegister: async (email, password) => {
    ...
    // --- FIX: Redirección Correcta al Cerrar Sesión ---
    performLogout: async () => {
        const res = await Auth.logout();
        if (res.success) {
            const path = window.location.pathname;
            // AQUI ESTÁ EL CAMBIO: Incluimos cuenta.html en la lista de redirección
...
window.AuthModal = {
    init: () => {
        ...
        if (lForm) lForm.onsubmit = async (e) => { 
            e.preventDefault(); 
            await AppUI.performLogin(document.getElementById('m-login-email').value, document.getElementById('m-login-pass').value);
        };
        
        const rForm = document.getElementById('form-modal-register');
        if (rForm) rForm.onsubmit = async (e) => { 
            e.preventDefault(); 
            await AppUI.performRegister(document.getElementById('m-reg-email').value, document.getElementById('m-reg-pass').value);
        };
    },
    open: (tab = 'login') => {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // --- MANEJO ROBUSTO DEL LOGIN EN PANEL ---
    const staticLoginForm = document.getElementById('panel-login-form');
    if (staticLoginForm) staticLoginForm.onsubmit = async (e) => {
        e.preventDefault();
        // Usamos AppUI.performLogin en lugar de Auth.login directo para asegurar el refresh
        await AppUI.performLogin(document.getElementById('login-email').value, document.getElementById('login-password').value);
    };
    ...
    if(pReg) pReg.onsubmit = async (e) => {
        e.preventDefault();
        await AppUI.performRegister(document.getElementById('registro-email').value, document.getElementById('registro-password').value);
    };
